...
serve(async (req) => {
    ...
    try {
        ...
        if (!resp.ok) {
            ...
            return new Response(JSON.stringify({ sent: false, status: resp.status }), {
                status: 200,
                headers: { 'Content-Type': 'application/json' },
            });
        }
        ...
        return new Response(JSON.stringify({ sent: true, heure }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        });
    } catch (err) {
        console.error('[notify-bisou] Network error', err);
        return new Response(JSON.stringify({ sent: false, error: 'network' }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        });
    }
