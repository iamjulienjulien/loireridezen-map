...
async function _doLoad() {
    ...
    const photoFeatures = (photosFC?.features ?? []).filter((f) => !f.properties?.poi_id);
    ...
    if (!_centerListenerAdded) {
        ...
        map.getContainer().addEventListener('click', (e) => {
            ...
            if (instaBtn) {
                e.preventDefault();
                trackAndNavigate('Step Instagram', instaBtn.href, {
                    step_id: instaBtn.dataset.stepId || '',
                });
                return;
            }
            ...
            if (komootBtn) {
                e.preventDefault();
                trackAndNavigate('Step Komoot', komootBtn.href, {
                    step_id: komootBtn.dataset.stepId || '',
                });
            }
        });
    }
            ...
            layers.push(
                new GeoJSON(data, {
                    ...
                    onEachFeature(feature, layer) {
                        ...
                        layer.bindPopup(renderStepPopup(matched, group), {
                            maxWidth: 300,
                            closeButton: false,
                        });
                        ...
                        layer.on('click', () =>
                            track('Step Opened', { step_id: matched.id, act: group.id })
                        );
                    },
                })
            );
