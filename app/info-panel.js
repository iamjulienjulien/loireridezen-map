...
function findCurrentOrNextStep(items) {
    ...
    const inProgress = sorted.find((t) => t.date_status === 'effective' && t.date === today);
    if (inProgress) return { trace: inProgress, status: 'en-cours' };

    const next = sorted.find((t) => t.date_status === 'planned' || t.date_status === 'prévue');
...
function renderStepBlock(info) {
    ...
    const badgeClass = isActive ? 'lrz-info-block__badge--active' : 'lrz-info-block__badge--next';
