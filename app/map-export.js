...
function drawCityLabels(
    ...
    for (let i = 0; i < markers.length; i++) {
        ...
        const obstacles = [...markerBoxes.filter((b, j) => b !== null && j !== i), ...placed];
...
async function renderToCanvas(
    ...
    const tracesToDraw =
        options.showDashed === false ? traces.map((t) => ({ ...t, dashed: false })) : traces;
    ...
    if (options.stats && statsData) {
        ...
        const hasContent =
            filteredLines.length > 0 || (options.showTitle !== false && filteredStats.title);
        if (hasContent) {
...
function _buildHTML(groups, tracesData) {
    ...
    const themes = THEMES.map(
        (t) => `<button class="lrz-export-theme" data-theme="${t.key}">${t.label}</button>`
    ).join('');
...
function _sel() {
    ...
    const eurovelo = _overlay.querySelector('#exp-opt-eurovelo')?.checked ?? false;
...
async function _renderPreview() {
    ...
    try {
        ...
        const previewTraces =
            options.showDashed === false ? data.traces.filter((t) => !t.dashed) : data.traces;
