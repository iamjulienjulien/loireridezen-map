...
const BASEMAPS = {
  osm: {
    label: "Plan OSM",
    tileUrl: (z, x, y) =>
      `https://${"abc"[(x + y + z) % 3]}.tile.openstreetmap.org/${z}/${x}/${y}.png`,
  ...
  cyclosm: {
    label: "CyclOSM",
    tileUrl: (z, x, y) =>
      `https://${"abc"[(x + y + z) % 3]}.tile-cyclosm.openstreetmap.fr/cyclosm/${z}/${x}/${y}.png`,
  ...
  topo: {
    label: "OpenTopo",
    tileUrl: (z, x, y) =>
      `https://${"abc"[(x + y + z) % 3]}.tile.opentopomap.org/${z}/${x}/${y}.png`,
...
function _formatPeriodFR(dates) {
  ...
  try {
    ...
    if (
      ...
      const monthYear = last.toLocaleDateString("fr-FR", {
        month: "long",
        year: "numeric",
      });
...
function _computeStatsData(mode, group, loaded, color) {
  if (mode === "act") {
    ...
    const totalDist = loaded.reduce(
      (s, { item }) => s + (item.distance_km ?? 0),
      0,
    );
    const totalElev = loaded.reduce(
      (s, { item }) => s + (item.elevation_gain_m ?? 0),
      0,
    );
    ...
    if (hasDist)
      lines.push({ text: `${_fmtNum(totalDist)} km`, icon: "route" });
    if (hasElev)
      lines.push({
        text: `${_fmtNum(totalElev)} m de dénivelé`,
        icon: "mountain",
      });
    ...
    if (item.date)
      lines.push({
        text: _formatDateFR(item.date) ?? item.date,
        icon: "calendar",
      });
...
async function drawBasemap(
  ...
  for (let tx = txMin; tx <= txMax; tx++) {
    for (let ty = tyMin; ty <= tyMax; ty++) {
      jobs.push(
        loadTile(tileUrlFn(zoom, tx, ty)).then((img) => ({ img, tx, ty })),
      );
    }
  }
...
function drawEuroVelo(ctx, geojson, zFloat, originWX, originWY, lineWidth) {
  ...
  const evWidth = Math.max(2, Math.round(lineWidth * 0.35));
  ctx.save();
  ctx.globalAlpha = 0.5;
  ctx.strokeStyle = "#6b7280";
  ctx.lineWidth = evWidth;
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ctx.setLineDash([]);
...
function drawStats(
  ...
  const boxH =
    pad +
    (showTitle && title ? titleFs + divH : 0) +
    lines.length * lineH +
    pad;
...
async function drawCurrentPosition(
  ctx,
  zFloat,
  originWX,
  originWY,
  ...
  try {
    const r = await fetch("data/catalog/current_position.json", {
      cache: "no-cache",
    });
    if (!r.ok) return;
...
async function loadSelectionData(mode, selectedId, groups, tracesData) {
  ...
  const color =
    typeof group?.color === "string" && !group.color.startsWith("fn:")
      ? group.color
      : "#3a6f8f";
...
async function renderToCanvas(
  canvas,
  { traces, markers, bbox, statsData, formatKey, basemapKey, options = {} },
) {
  ...
  const { zoom, tileScale, zFloat, originWX, originWY } = computeRenderParams(
    bbox,
    fmt.w,
    fmt.h,
    0.05,
  ...
  if (options.eurovelo6) {
    const [ev6, ev1] = await Promise.all([
      fetchGeoJSON("data/eurovelo/eurovelo-6.geojson"),
      fetchGeoJSON("data/eurovelo/eurovelo-1.geojson"),
    ]);
    if (ev6) drawEuroVelo(ctx, ev6, zFloat, originWX, originWY, lineWidth);
    if (ev1) drawEuroVelo(ctx, ev1, zFloat, originWX, originWY, lineWidth);
  }
  ...
  if (options.stats && statsData) {
    ...
    const hasContent =
      ...
      (options.showTitle !== false && filteredStats.title);
    if (hasContent) {
...
function _buildHTML(groups, tracesData) {
  const gItems = (groups.items ?? []).sort(
    (a, b) => (a.order ?? 0) - (b.order ?? 0),
  );
  const tItems = tracesData.items ?? [];

  const actOpts = gItems
    .map((g) => `<option value="${g.id}">${g.label}</option>`)
    .join("");
  const stepOpts = gItems
    .map((g) => {
      const its = tItems
  ...
  const basemapBtns = Object.entries(BASEMAPS)
    .map(
      ([key, bm]) =>
    ...
    <div class="lrz-export-modal__body">
      ...
      <div class="lrz-export-section">
        ...
        <div class="lrz-export-options">
          ...
          <label class="lrz-export-opt">
            <input type="checkbox" id="exp-opt-eurovelo">
            <span class="lrz-export-opt__label">Afficher les EuroVelos (1 &amp; 6)</span>
          </label>
          <label class="lrz-export-opt">
...
function _sel() {
  ...
  const fmt =
    _overlay.querySelector(".lrz-export-fmt.is-active")?.dataset.fmt ??
    "square";
  const bm = _overlay.querySelector('[name="exp-bm"]:checked')?.value ?? "osm";
  const color =
    _overlay.querySelector(".lrz-export-swatch.is-active")?.dataset.color ??
    COLOR_PALETTE[0].hex;
  const font =
    _overlay.querySelector(".lrz-export-font-btn.is-active")?.dataset.font ??
    "Geist";
  const showDashed =
    _overlay.querySelector("#exp-opt-upcoming")?.checked ?? true;
  ...
  const eurovelo =
    _overlay.querySelector("#exp-opt-eurovelo")?.checked ?? false;
...
async function _renderPreview() {
  ...
  try {
    const data = await loadSelectionData(
      mode,
      selectedId,
      _groups,
      _tracesData,
    ...
    if (options.eurovelo6) {
      ...
      if (ev1) drawEuroVelo(ctx, ev1, zFloat, originWX, originWY, 2);
    }
    ...
    if (options.stats && data.statsData) {
      ...
      const hasContent =
        filteredLines.length > 0 || (options.showTitle && filteredStats.title);
      if (hasContent) {
        drawStats(
          ctx,
          previewW,
    ...
    if (options.position)
      await drawCurrentPosition(ctx, zFloat, originWX, originWY, data.bbox, 20);
...
async function _generate() {
  ...
  try {
    ...
    const slug = (selectedId ?? "export")
      .replace(/[^a-z0-9_-]/gi, "-")
      .toLowerCase();
    const url = URL.createObjectURL(blob);
    Object.assign(document.createElement("a"), {
      href: url,
      download: `loire-ride-zen_${slug}_${formatKey}.jpg`,
    }).click();
    setTimeout(() => URL.revokeObjectURL(url), 8000);
    btn.textContent = "Téléchargé ✓";
    setTimeout(() => {
      if (btn) btn.textContent = "Générer et télécharger";
    }, 3000);
  } catch (err) {
...
export async function openExportModal() {
  ...
  _overlay
    .querySelector(".lrz-export-modal__close")
  ...
  _overlay
    .querySelector("#exp-act")
    .addEventListener("change", _schedulePreview);
  _overlay
    .querySelector("#exp-step")
    .addEventListener("change", _schedulePreview);
  ...
  _overlay.querySelectorAll(".lrz-export-fmt").forEach((btn) => {
    btn.addEventListener("click", () => {
      _overlay
        .querySelectorAll(".lrz-export-fmt")
  ...
  _overlay.querySelectorAll(".lrz-export-swatch").forEach((btn) => {
    btn.addEventListener("click", () => {
      _overlay
        .querySelectorAll(".lrz-export-swatch")
        .forEach((b) => b.classList.remove("is-active"));
      btn.classList.add("is-active");
  ...
  _overlay.querySelectorAll(".lrz-export-theme").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (!theme) return;
      // Fond
      const bmRadio = _overlay.querySelector(
        `[name="exp-bm"][value="${theme.basemap}"]`,
      );
      ...
      _overlay
        .querySelector(`.lrz-export-swatch[data-color="${theme.color}"]`)
        ?.classList.add("is-active");
      // Police
      _overlay
        .querySelectorAll(".lrz-export-font-btn")
        .forEach((b) => b.classList.remove("is-active"));
      _overlay
        .querySelector(`.lrz-export-font-btn[data-font="${theme.font}"]`)
        ?.classList.add("is-active");
      // Thème actif
      _overlay
        .querySelectorAll(".lrz-export-theme")
        .forEach((b) => b.classList.remove("is-active"));
      btn.classList.add("is-active");
      _schedulePreview();
    });
  ...
  // Options checkboxes
  _overlay
    .querySelectorAll('.lrz-export-opt input[type="checkbox"]')
    .forEach((cb) => {
      cb.addEventListener("change", _schedulePreview);
    });
  ...
  // Étapes à venir
  _overlay
    .querySelector("#exp-opt-upcoming")
    ?.addEventListener("change", _schedulePreview);
...
export function initExportButton() {
  ...
  grp.innerHTML =
    '<button class="lrz-apanel-btn" title="Exporter en image" aria-label="Exporter en image">📷</button>';
