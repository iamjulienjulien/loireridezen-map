...
function renderEditorialPoiPopup(p) {
  ...
  const attachedSrc = attached.length
    ...
    : null;
  const photo =
    p.photo_path || safeHttpUrl(p.thumb) || safeHttpUrl(p.image) || p.thumb || p.image || attachedSrc || null;
  return `
...
function ensureErrorBanner() {
  ...
  div.querySelector(".lrz-error-banner__retry").addEventListener("click", () => loadPoisForViewportGL());
...
function addPoiMarker(feature) {
  ...
  el.addEventListener("click", (ev) => {
    ...
    if (customClose) {
      root?.querySelector(".lrz-popup__close")?.addEventListener("click", () => popup.remove());
    }
