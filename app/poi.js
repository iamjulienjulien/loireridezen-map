...
import { LayerGroup, GeoJSON, Marker, Control, DomUtil, DomEvent } from 'leaflet';
...
errorControl.onAdd = function () {
  ...
  `;
    ...
    div.querySelector('.lrz-error-banner__retry').addEventListener('click', () => currentRetry?.());
    return div;
};
...
export async function loadPoisForViewport() {
    ...
    if (hiddenModes.rabbit) {
        ...
        activeTypes = Array.from(document.querySelectorAll('.type-filter:checked')).map(
            (i) => i.value
        );
